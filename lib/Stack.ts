...
export class IamStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    // Create Policies
    const AwsPolicies: ManagedPolicy[] = CdkManagedPolicies.map((policy) =>
      CdkPolicy.Set(policy),
    );
    ...
    AwsPolicies.forEach((AwsPolicy) =>
      CdkPolicy.AddPolicyToAssignedGroups(AwsPolicy),
    );
